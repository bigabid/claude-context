...
- `handlers.ts` (`ToolHandlers`) — implements the tools: `index_codebase`, `search_code`, `clear_index`, `get_indexing_status`, `list_indexed_repos`, `search_repo` (search one repo by git identity, no local checkout needed), `search_org` (fan out across every indexed repo at once). In `index.ts`, `setupTools()` hides `index_codebase`/`clear_index`/`get_indexing_status`/`search_code` from the tool list (and rejects them if called anyway) when `transport === 'http'` — all four require a local filesystem path, which the shared HTTP server doesn't have. `search_code` is the subtle one: its collection lookup reads `.git/config` from the given path to resolve the git-remote identity, but that path only exists on the *caller's* machine, not the server, so over HTTP it would silently mis-hash and always report "not indexed" rather than just being a worse choice than `search_repo`/`search_org`.
